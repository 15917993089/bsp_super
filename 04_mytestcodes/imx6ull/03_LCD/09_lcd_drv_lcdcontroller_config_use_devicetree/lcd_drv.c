/* 添加了新内容platform_driver */
/* 添加了 static struct gpio_desc *b1_goio相关内容 */
/* 添加了static struct clk *clk_pix;
static struct clk *clk_axi;参考了内核自带的设备树文件和驱动文件修改了
自己的设备树和驱动，在自己的设备树里面添加了pix和axi时钟（使用时钟子系统） */
/* 添加修改驱动程序从设备树里面获得LCD的时序和极性参数，简单来说就是在设备树
里面指定LCD参数，从设备树里面获取参数*/
/* LCD控制器本身 */
...
static struct gpio_desc *b1_goio;
...
static int mylcd_probe(struct platform_device *pdev)
{
	...
	display_np = of_parse_phandle(pdev->dev.of_node, "display", 0);//从设备树结点找到属性
	/* get common info */
	...
	b1_gpio = gpiod_get(&pdev->dev, "backlight", 0);
	...
	clk_pix = devm_clk_get(&pdev->dev, "pix");//我去设备里面对应的设备结点获得名为pix的clk
												//并保存在前面那里
	clk_axi = devm_clk_get(&pdev->dev, "axi");

	/* set clk rate */
	clk_set_rate(clk_pix,50000000);//以后这个50M应该从设备树里面获得，这里只是一个例子
												
	/* enable clk */
	clk_prepare_enable(clk_pix);
	clk_prepare_enable(clk_axi);//到这里只是示例代码还不能运行~~~
...
//以前是分配设置注册分fb_info，现在是platform设备树操作
static const struct of_device_id mylcd_of_match[] = {
	{ .compatible = "100ask,mylcd_drv", },//这个才重要！！！
	{ },
};
...
//之所以init和exit的操作是变成这样子是因为有更大的框架，
//更大的框架platform融合了容纳了更小的框架mylcd
...
#if 0
	framebuffer@0 {
                        compatible = "100ask,mylcd_drv";
						pinctrl-names = "default";//引脚名为默认
						pinctrl-0 = <&mylcd_pinctrl>;//这两步足够设置某些引脚为某些功能
						//还要设置配置某些引脚为输出功能
						backlight-gpios = <&gpio1 8 GPIO_AVTIVE_HIGH>;
	};
