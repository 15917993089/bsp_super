...
void dc_pin_init(int number)//引脚初始化
{
...
void oled_set_dc_pin(int val)//设置引脚的数据
{
 ...
 ***********************************************************************/
void OLED_DIsp_Set_Pos(int x, int y)
{ 	oled_write_cmd_data(0xb0+y,OLED_CMD);//设置页的
	oled_write_cmd_data((x&0x0f),OLED_CMD); //列的低4位
	oled_write_cmd_data(((x&0xf0)>>4)|0x10,OLED_CMD);//列的高4位
}   	      	   			 
/**********************************************************************
	  * 函数名称： OLED_DIsp_Char
	  * 功能描述：在某个位置显示字符 1-9
...
int main(int argc, char **argv)
{
	...
	dc_pin = strtoul(argv[2], NULL, 0);//转换为一个引脚的编号
	dc_pin_init(dc_pin);
